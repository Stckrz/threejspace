...
export class SpherePlanet {
	...
	create() {
		...
		const sphereCubeGeometry = new THREE.SphereGeometry(80, 80, 80)
		const sphereCubeMaterial = new THREE.MeshBasicMaterial({ color: 0xFFFFFF, vertexColors: true })
	...
	changeScale() {
		...
		for (let i = 0; i < instanceCount; i++) {
			const scale = (this.dataArray[i] / 255) * 1 + 0.5;
